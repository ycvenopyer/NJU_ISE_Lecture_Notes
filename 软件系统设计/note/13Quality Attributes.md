...
[](#11-架构扮演的角色-role-of-architecture)[](#11-架构扮演的角色-role-of-architecture)1.1. 架构扮演的角色 Role of Architecture
---------------------------------------------------------------------------------------------------------
...
[](#12-为什么软件架构是重要的-why-is-software-architecture-important)[](#12-为什么软件架构是重要的-why-is-software-architecture-important)1.2. 为什么软件架构是重要的 Why is software architecture important?
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
7.  架构将**更改**分为三种类型 Architecture categorise changes into three types:
    1.  **本地**：单一组件修改 Local: single component modification
    2.  **非本地**：几个组件修改。Non-local: several component modification
...
10.  可以通过体系结构集成独立开发的组件来开发系统 (基于 Component 的软件工程 - CBSE) Systems can be developed by integrating independently developed components via architecture ((Component-Based Software Engineering - CBSE)

[](#13-软件架构过程-software-architecture-process)[](#13-软件架构过程-software-architecture-process)1.3. 软件架构过程 Software Architecture Process
---------------------------------------------------------------------------------------------------------------------------------
...
2.  通过分析得到 Prioritized Quality Attribute Scenarios(高优先级质量属性解决场景) 和 Requirements，Constraints(需求和约束)
...
### [](#131-移动手机系统架构-mobile-phone-system-architecture)[](#131-移动手机系统架构-mobile-phone-system-architecture)1.3.1. 移动手机系统架构 Mobile Phone System Architecture

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/2.png)

### [](#132-洗衣机架构-washing-machine-architecture)[](#132-洗衣机架构-washing-machine-architecture)1.3.2. 洗衣机架构 Washing Machine Architecture

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/3.png)

[](#14-讨论-discussion)[](#14-讨论-discussion)1.4. 讨论 Discussion
------------------------------------------------------------
...
[](#21-功能性需求-functional-requirements)[](#21-功能性需求-functional-requirements)2.1. 功能性需求 Functional Requirements
------------------------------------------------------------------------------------------------------------
...
[](#22-质量需求-quality-requirements)[](#22-质量需求-quality-requirements)2.2. 质量需求 Quality Requirements
------------------------------------------------------------------------------------------------
...
### [](#221-非功能性需求-non-functional-requirements)[](#221-非功能性需求-non-functional-requirements)2.2.1. 非功能性需求 Non-functional Requirements
...
4.  非功能性需求分为两大类：There are two broad categories of non-functional requirements:
...
### [](#222-质量属性-quality-attributes)[](#222-质量属性-quality-attributes)2.2.2. 质量属性 Quality Attributes
...
### [](#223-确定质量属性-specifying-quality-attributes)[](#223-确定质量属性-specifying-quality-attributes)2.2.3. 确定质量属性 Specifying Quality Attributes
...
2.  质量属性**场景**用于定义所需的质量属性。Quality attribute scenarios are used to define the desired quality attribute.
3.  场景是具有一定结构的简单句子。场景的两个主要类别是：Scenarios are simple descriptions with certain structure. Two main classes of scenarios are:
    1.  **通用场景**是与**系统无关**的场景，用于指导质量属性要求的规范 General scenarios are system independent scenarios to guide the specification of quality attribute requirements.
    2.  **具体场景**是系统**特定**场景，用于指导特定系统的质量属性要求的规范。它们是一般场景的**实例** Concrete scenarios are system specitic scenarios to guide the specification of quality attribute requirements tor a particular system. They are instances of general scenarios.
4.  这个场景就是 4+1 视图中的 1(Use Case)

#### [](#2231-通用场景-general-scenarios)[](#2231-通用场景-general-scenarios)2.2.3.1. 通用场景 General Scenarios

1.  通用场景提供了一个**框架**，用于生成**大量**通用的，独立于系统的，质量属性特定的场景。General scenarios provide a framework for generating a large number of generic, system- independent, quality attribute specific scenarios.
...
#### [](#2232-质量属性场景建模-modeling-quality-attribute-scenarios-重要)[](#2232-质量属性场景建模-modeling-quality-attribute-scenarios-重要)2.2.3.2. 质量属性场景建模 Modeling Quality Attribute Scenarios 重要
...
3.  响应 (Response)：刺激措施到来之后开展的**活动** Response: The activity undertaken after the arrival of the stimulus.
...
#### [](#2233-tactics-策略原子级别的最小的决定)[](#2233-tactics-策略原子级别的最小的决定)2.2.3.3. Tactics 策略 (原子级别的最小的决定)
...
#### [](#2234-质量设计决策-quality-design-decisions)[](#2234-质量设计决策-quality-design-decisions)2.2.3.4. 质量设计决策 Quality Design Decisions
...
[](#23-约束-constraints)[](#23-约束-constraints)2.3. 约束 Constraints
---------------------------------------------------------------
...
[](#31-可用性-availability)[](#31-可用性-availability)3.1. 可用性 Availability
---------------------------------------------------------------------
...
2.  度量方式：以**所需**的可用**时间**比例来衡量，例如 Measured by the proportion of the required time it is useable, e.g.
    ...
    2.  每周计划的停机时间不超过 2 个小时 - 24x7x52(100％可用性) No more than 2 hours scheduled downtime per week - 24x7x52 (100% availability)
...
### [](#311-outage-failure-fault-error)[](#311-outage-failure-fault-error)3.1.1. Outage, Failure, Fault, Error
...
### [](#314-可用性通用场景-availability-general-scenario)[](#314-可用性通用场景-availability-general-scenario)3.1.4. 可用性通用场景 Availability General Scenario
...
### [](#315-可用性具体场景-availability-sample-scenario)[](#315-可用性具体场景-availability-sample-scenario)3.1.5. 可用性具体场景 Availability Sample Scenario
...
### [](#316-可用性策略-availability-tactics)[](#316-可用性策略-availability-tactics)3.1.6. 可用性策略 Availability Tactics
...
### [](#317-fault)[](#317-fault)3.1.7. Fault

#### [](#3171-fault-探测-fault-detection)[](#3171-fault-探测-fault-detection)3.1.7.1. Fault 探测 Fault Detection
...
#### [](#3172-fault-恢复-fault-recovery)[](#3172-fault-恢复-fault-recovery)3.1.7.2. Fault 恢复 Fault Recovery
...
### [](#318-可用性设计和分析的检查列表-checklist-for-availability-design-analysis)[](#318-可用性设计和分析的检查列表-checklist-for-availability-design-analysis)3.1.8. 可用性设计和分析的检查列表 Checklist for Availability Design & Analysis

<table><thead><tr><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/14.png" style="width: auto;"></th><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/15.png"></th></tr></thead><tbody></tbody></table>

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/60.png)

[](#32-互操作性-interoperability)[](#32-互操作性-interoperability)3.2. 互操作性 Interoperability
------------------------------------------------------------------------------------
...
### [](#321-互操作性的通用场景-interoperability-general-scenario)[](#321-互操作性的通用场景-interoperability-general-scenario)3.2.1. 互操作性的通用场景 Interoperability General Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/16.png)

### [](#322-互操作性的样本场景-interoperability-sample-scenario)[](#322-互操作性的样本场景-interoperability-sample-scenario)3.2.2. 互操作性的样本场景 Interoperability Sample Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/17.png)

### [](#323-互操作性的策略-interoperability-tactics)[](#323-互操作性的策略-interoperability-tactics)3.2.3. 互操作性的策略 Interoperability Tactics
...
### [](#324-互操作性的检查列表-checklist-for-interoperability-design-analysis)[](#324-互操作性的检查列表-checklist-for-interoperability-design-analysis)3.2.4. 互操作性的检查列表 Checklist for Interoperability Design & Analysis

<table><thead><tr><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/19.png" style="width: auto;"></th><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/20.png"></th></tr></thead><tbody></tbody></table>

[](#33-可修改性-modifiability)[](#33-可修改性-modifiability)3.3. 可修改性 Modifiability
---------------------------------------------------------------------------
...
5.  计算公式：N * 不使用机械装置进行更改的成本 ≤ 安装机械装置的成本 +(N * 使用机械装置进行更改的成本)
6.  降低的成本可以用于提高可修改性

### [](#331-可修改性的通用场景-modifiability-general-scenario)[](#331-可修改性的通用场景-modifiability-general-scenario)3.3.1. 可修改性的通用场景 Modifiability General Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/21.png)

### [](#332-可修改性的样本场景-modifiability-sample-scenario)[](#332-可修改性的样本场景-modifiability-sample-scenario)3.3.2. 可修改性的样本场景 Modifiability Sample Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/22.png)

### [](#333-可修改性的策略-modifiability-tactics)[](#333-可修改性的策略-modifiability-tactics)3.3.3. 可修改性的策略 Modifiability Tactics
...
### [](#334-checklist-for-modifiability-design-and-analsis)[](#334-checklist-for-modifiability-design-and-analsis)3.3.4. Checklist for Modifiability Design and Analsis

<table><thead><tr><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/24.png" style="width: auto;"></th><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/25.png" style="width: auto;"></th></tr></thead><tbody></tbody></table>

[](#34-性能-performance)[](#34-性能-performance)3.4. 性能 Performance
---------------------------------------------------------------
...
### [](#341-性能的通用场景-performance-general-scenario)[](#341-性能的通用场景-performance-general-scenario)3.4.1. 性能的通用场景 Performance General Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/26.png)

### [](#342-性能的样本场景-performance-sample-scenario)[](#342-性能的样本场景-performance-sample-scenario)3.4.2. 性能的样本场景 Performance Sample Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/27.png)

### [](#343-性能的策略-tactics-for-performance)[](#343-性能的策略-tactics-for-performance)3.4.3. 性能的策略 Tactics for Performance
...
### [](#344-checklist-for-performance-design-and-analysis)[](#344-checklist-for-performance-design-and-analysis)3.4.4. Checklist for Performance Design and Analysis

<table><thead><tr><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/29.png" style="width: auto;"></th><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/30.png"></th></tr></thead><tbody></tbody></table>

[](#35-安全性-security)[](#35-安全性-security)3.5. 安全性 Security
---------------------------------------------------------
...
### [](#351-安全性的通用方案-security-general-scenario)[](#351-安全性的通用方案-security-general-scenario)3.5.1. 安全性的通用方案 Security General Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/31.png)

### [](#352-安全性的样本方案-security-sample-scenario)[](#352-安全性的样本方案-security-sample-scenario)3.5.2. 安全性的样本方案 Security Sample Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/32.png)

### [](#353-安全性的策略-security-tactics)[](#353-安全性的策略-security-tactics)3.5.3. 安全性的策略 Security Tactics
...
### [](#354-安全性的检查列表-checklist-for-security-design-and-analysis)[](#354-安全性的检查列表-checklist-for-security-design-and-analysis)3.5.4. 安全性的检查列表 Checklist for Security Design and Analysis

<table><thead><tr><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/34.png" style="width: auto;"></th><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/35.png"></th></tr></thead><tbody></tbody></table>

[](#36-可测试性-testability)[](#36-可测试性-testability)3.6. 可测试性 Testability
---------------------------------------------------------------------
...
### [](#361-可测试性的通用场景-testability-general-scenario)[](#361-可测试性的通用场景-testability-general-scenario)3.6.1. 可测试性的通用场景 Testability General Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/37.png)

### [](#362-可测试性的样本场景-testability-sample-scenario)[](#362-可测试性的样本场景-testability-sample-scenario)3.6.2. 可测试性的样本场景 Testability Sample Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/38.png)

### [](#363-可测试性的策略-testability-tactcs)[](#363-可测试性的策略-testability-tactcs)3.6.3. 可测试性的策略 Testability Tactcs
...
### [](#364-可测试性的检查列表-checklist-for-testability-design-and-analysis)[](#364-可测试性的检查列表-checklist-for-testability-design-and-analysis)3.6.4. 可测试性的检查列表 Checklist for Testability Design and Analysis

<table><thead><tr><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/40.png" style="width: auto;"></th><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/41.png"></th></tr></thead><tbody></tbody></table>

[](#37-易用性-usability)[](#37-易用性-usability)3.7. 易用性 Usability
------------------------------------------------------------
...
### [](#371-易用性的通用场景-usability-general-scenario)[](#371-易用性的通用场景-usability-general-scenario)3.7.1. 易用性的通用场景 Usability General Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/42.png)

### [](#372-易用性的样本场景-usbility-sample-scenario)[](#372-易用性的样本场景-usbility-sample-scenario)3.7.2. 易用性的样本场景 Usbility Sample Scenario

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/43.png)

### [](#373-易用性的策略-usability-tactics)[](#373-易用性的策略-usability-tactics)3.7.3. 易用性的策略 Usability Tactics
...
### [](#374-checklist-for-usbility-design-and-analysis)[](#374-checklist-for-usbility-design-and-analysis)3.7.4. Checklist for Usbility Design and Analysis

<table><thead><tr><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/45.png" style="width: auto;"></th><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/46.png"></th></tr></thead><tbody></tbody></table>

[](#38-更多的能力-x-ability)[](#38-更多的能力-x-ability)3.8. 更多的能力 X-ability
------------------------------------------------------------------
...
[](#41-从需求文档中收集asr-gathering-asrs-from-requirements-documents)[](#41-从需求文档中收集asr-gathering-asrs-from-requirements-documents)4.1. 从需求文档中收集 ASR Gathering ASRs from Requirements Documents
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
2.  MoSCoW 样式：[MoSCoW 的样式说明](https://blog.csdn.net/cheny_com/article/details/6358456)：使用四个级别来定义一个需求的优先级程度
...
[](#42-通过和涉众面谈来收集asr-gathering-asrs-by-interviewing-stakeholders)[](#42-通过和涉众面谈来收集asr-gathering-asrs-by-interviewing-stakeholders)4.2. 通过和涉众面谈来收集 ASR Gathering ASRs by Interviewing Stakeholders
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
1.  质量属性工作坊 (QAW) Quality Attribute Workshop (QAW)
    ...
    6.  场景合并 (合并类似场景) Scenario consolidation (merging similar scenarios)
    7.  场景优先级 (通过投票) Scenario prioritization(by voting)
    8.  场景细化：对最重要的方案进行细化和阐述。Scenario refinement: the top scenarios are refined and elaborated.
2.  QAW 的结果包括架构驱动程序列表和利益相关者 (作为一个小组优先考虑) 的一组(2A 场景)。The results of QAW include a list of architectural drivers and a set of QA scenarios that the stakeholders (as a group prioritized).

[](#43-通过utility树来获取asr-capturing-asrs-in-a-utility-tree)[](#43-通过utility树来获取asr-capturing-asrs-in-a-utility-tree)4.3. 通过 Utility 树来获取 ASR Capturing ASRs in a Utility Tree
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
[](#44-基于决策来发现asr的方法-persona-based-approach-to-exploring-asrs)[](#44-基于决策来发现asr的方法-persona-based-approach-to-exploring-asrs)4.4. 基于决策来发现 ASR 的方法 Persona-Based Approach to exploring ASRs
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

### [](#441-结合asr工作-working-with-asrs)[](#441-结合asr工作-working-with-asrs)4.4.1. 结合 ASR 工作 Working with ASRs
...
### [](#443-考虑折中-competing-tradeoffs)[](#443-考虑折中-competing-tradeoffs)4.4.3. 考虑折中 Competing Tradeoffs

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/49.png)

### [](#444-传统的hci角色-traditional-hci-personas)[](#444-传统的hci角色-traditional-hci-personas)4.4.4. 传统的 HCI 角色 Traditional HCI Personas
...
2.  传统上，角色构建涉及对用户进行调查，对其进行分类，制定使用假设，进行验证，创建场景以及最终设计角色。Traditionally persona construction involves surveying users, classifying them, formulating hypotheses of use, validating, creating scenarios, and finally designing personas.
...
### [](#445-精通架构的角色精简版-architecturally-savvylite)[](#445-精通架构的角色精简版-architecturally-savvylite)4.4.5. 精通架构的角色 (精简版) Architecturally-Savvy(Lite)

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/50.png)

### [](#446-几个例子)[](#446-几个例子)4.4.6. 几个例子

<table><thead><tr><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/51.png" style="width: auto;"></th><th><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/52.png"></th></tr></thead><tbody><tr><td><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/53.png"></td><td><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/54.png"></td></tr><tr><td><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/55.png"></td><td><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/56.png"></td></tr><tr><td><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/57.png"></td><td><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/58.png"></td></tr><tr><td><img class="" src="https://spricoder.oss-cn-shanghai.aliyuncs.com/2021-Software-System-Design/img/lec13/59.png"></td><td></td></tr></tbody></table>
